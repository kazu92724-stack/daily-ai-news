...
def summarize_medical_trends():
    med_data = fetch_rss_news("医療 検査 ゲノム 病理", max_items=10)
    local_data = fetch_rss_news("和歌山 医療 OR 大阪 病院 開業", max_items=5)
    ...
    prompt = (
        ...
        "・各トピックには必ず元のURLを記載し、全体で500〜600字程度に収めてください（前置き不要）。"
    )
...
# --- ntfy.sh 送信関数（添付ファイル化を自動防止） ---
def send_notification(title, text):
    # ntfyの文字数制限（バイト制限）対策のため、約1,000文字を超える場合はカット
    if len(text) > 1000:
        text = text[:950] + "\n\n(※文字数制限のため一部省略)"
